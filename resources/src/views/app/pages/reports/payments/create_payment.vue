<template>
  <div class="main-content">
    <breadcumb :page="$t('AddPayment')" :folder="$t('Payment_Purchases')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="create_payment" v-if="!isLoading">
          ...
          <b-col lg="12" md="12" sm="12">
              ...
              <b-row>
                ...
                <b-col lg="12" md="12" sm="12">
                  <b-form-group :label="$t('date')">
                    <b-form-input type="date" v-model="paymentHeader.date" class="header-input"></b-form-input>
                  </b-form-group>
                </b-col>
              </b-row>
            </b-card>
            ...
            <div class="excel-container mt-4">
              <div class="d-flex justify-content-between mb-3 align-items-center">
                 <h3 class="mb-0 text-dark font-weight-bold">Payment Worksheet (Grid View)</h3>
              </div>
              ...
              <div class="table-responsive worksheet-grid-wrapper">
                <table class="table table-bordered excel-grid">
                  <thead>
                    <tr>
                      <th style="width: 35%;">{{$t('Supplier')}}</th>
              ...
              <div class="mt-4 text-right">
                ...
                >
                  <i class="i-Yes mr-2"></i> SAVE ALL PAYMENTS
                </b-button>
              </div>
...
export default {
  ...
  methods: {
    loadData() {
      axios.get("payment_purchase/create")
        .then(response => {
          this.suppliers = response.data.suppliers || [];
          this.payment_methods = response.data.payment_methods || [];
          this.accounts = response.data.accounts || [];
          this.unpaid_purchases = response.data.unpaid_purchases || [];
          ...
          if (this.accounts.length > 0) {
            ...
            this.paymentHeader.account_id = cashAccount ? cashAccount.id : this.accounts[0].id;
          }
          this.paymentHeader.payment_method_id = 2; // Default to Cash (ID 2)
    ...
    onSupplierChange(index) {
      ...
      if (row.supplier_id) {
        ...
        const list = this.unpaid_purchases.filter(p => p.provider_id === row.supplier_id);
        const totalPending = list.reduce((sum, p) => sum + (parseFloat(p.GrandTotal) - parseFloat(p.paid_amount)), 0);
    ...
    focusCell(index, fieldName) {
      if (fieldName === 'supplier') {
        const refName = `row_supplier_${index}`;
    ...
    submitSpreadsheet() {
      const validRows = this.spreadsheetRows.filter(row => row.supplier_id && row.amount > 0);
      ...
      const paymentsData = validRows.map(row => {
        return {
          ...
          account_id: this.paymentHeader.account_id,
          payment_method_id: this.paymentHeader.payment_method_id,
          date: this.paymentHeader.date,
      ...
      .then(response => {
        NProgress.done();
        this.makeToast("success", "All payments saved successfully", "Success");
        this.$router.push({ name: "index_payments" });
      })
      .catch(error => {
        NProgress.done();
        this.is_bulk_processing = false;
        this.makeToast("danger", error.response?.data?.message || "Failed to save payments", "Error");
      });
    },
